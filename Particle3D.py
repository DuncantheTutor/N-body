...
class Particle3D(object):
    ...
    * initialize particles from input file
    ...
    def xyz__str__(self):
        """
        new function to write .xyz files for trajectory simulations 
        return label x y z
    ...
    def init_from_file(filename,l):
       ...
       """
       # lines in file should be written as: label,mass,pos[0 -> 2], vel[0 -> 2]
       ...
       label = choose[0]    # read mass, label
       mass = float(choose[1])
       for i in xrange(2,5):   #read pos and vel
           pos[i-2] = float(choose[i])
       for j in xrange(5,8):
           vel[j-5] = float(choose[j])
